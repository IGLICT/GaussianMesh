...
# from edit.graphics_utils import getWorld2View2
...
def generate_ellipse_path(views, n_frames=600, const_speed=True, z_variation=0.5, z_phase=0.):
    poses = []
    for view in views:
        tmp_view = np.eye(4)
        tmp_view[:3] = np.concatenate([view.R.T, view.T[:, None]], 1)
    ...
    def get_positions(theta):
        ...
        return np.stack([
            ...
            z_variation * (z_low[2] + (z_low - z_low)[2] *
...
def gaussian_poses(viewpoint_cam, mean=0, std_dev_translation=0.03, std_dev_rotation=0.01):
    ...
    viewpoint_cam.world_view_transform = jt.array(getWorld2View2(viewpoint_cam.R, viewpoint_cam.T, translate)).transpose(0, 1).cuda()
    viewpoint_cam.full_proj_transform = jt.nn.bmm(viewpoint_cam.world_view_transform.unsqueeze(0),viewpoint_cam.projection_matrix.unsqueeze(0)).squeeze(0)
    viewpoint_cam.camera_center = jt.linalg.inv(viewpoint_cam.world_view_transform)[3, :3]
